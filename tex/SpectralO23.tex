...
\begin{document}
  ...
  \begin{abstract}
    O21 defined the observable shell $n_{3}$ by the threshold $\Sigma_{c}(n_{3}) = 3$ and used it
    as structural input.
    This paper determines the exact status of the value three.
    The positive result is conditional: if the admissible neutral sector is carried by an
    irreducible two-dimensional $\SU(2)$-valued $V_{\rho} \cong \mathbb{C}^{2}$, its traceless
    neutral sector is $\su(2) \cong \ImH$, of real dimension exactly three.
    Two bridges separate this theorem from a derivation of the threshold, and both are open.

    On the first, carrier selection, the paper determines what one candidate source supplies.
    The fingerprint vectors of the deposited O12 filtration are pure Fourier modes, so every
    proper level is a coordinate subspace with single-line Weyl support, and the exact stabiliser
    of that level in the Weil image of $\SL(2,\mathbb{Z}/q\mathbb{Z})$ is $U \rtimes M_{n}$,
    inside a Borel subgroup.
    This excludes the exceptional binary polyhedral groups $2T$, $2O$ and $2I$ at every proper
    level, including at primes where they exist in the ambient group.
    It does not exclude a spinor carrier: at explicit generic levels at $q = 53$ and $q = 101$
    the stabiliser is $\mathrm{Dic}_{q}$, whose restricted Weil module is multiplicity-free, one
    character plus four inequivalent two-dimensional irreducibles, exactly two of them faithful
    and $\SU(2)$-valued.
    The source therefore supplies genuine spinorial content, and the difficulty is selection, not
    existence.
    It splits in two: selecting which prime, block and depth the projection singles out within
    the generic filtration, which nothing here addresses; and, inside a level already known to be
    dicyclic, choosing between the two admissible carriers, for which a structurally motivated
    candidate --- the unique odd member of $\{c_{\Sigma}, q - c_{\Sigma}\}$ --- succeeds on all
    $17$ audited instances over $13$ distinct subspaces, with one step observed and not proved.

    The second bridge is observable identification: no theorem identifies the cumulative
    Gram--Schmidt span $\Sigma_{c}$ with the dimension of the neutral traceless module, so the
    threshold remains a supplied selection rule.
    A caution accompanies the positive result: in the faithful two-dimensional representation of
    $Q_{8}$ the set $\{\pm\mathbf{i}, \pm\mathbf{j}\}$ generates the group with only two
    generator axes.
    The ADE observation of three eigenvalue classes is retained as a consistency check, not as
    evidence.
  \end{abstract}
  ...
  \subsection{The adjoint-dimension theorem}
    ...
    \begin{remark}[What carries the ``3'']
      ...
      $\mathbb{C}^{2}$.
      The relation to the symmetric square requires its real form to be stated: multiplication
      by $\mathrm{i}$ identifies $\operatorname{Herm}_{0}(V_{\rho})$ with $\su(2)$ as real
      adjoint modules, both of real dimension three, and their complexification is isomorphic
      to $\operatorname{Sym}^{2}(V_{\rho})$, which has complex dimension three and underlying
      real dimension six.
      Writing $\su(2) \cong \operatorname{Sym}^{2}(V_{\rho})$ without that qualification
      conflates a real with a complex dimension.
  ...
  \subsection{The honest chain}
    ...
    Subsection~\ref{ssec:o12-obstruction} analyses the second arrow for one candidate source,
    the deposited O12 shell-span filtration, and refines it into an existence part that the
    source does supply and a selection part that it does not.

  \subsection{What the O12 filtration supplies on Bridge 1, and what it does not}
    ...
    spinor module.
    This subsection determines exactly what that candidate supplies.
    The answer is neither ``nothing'' nor ``a carrier'': the exceptional binary polyhedral
    groups are excluded at every proper level, while at explicit levels the stabiliser is
    dicyclic and the level carries genuine spinorial content, without any derived rule
    distinguishing one admissible carrier from the other.

    Fix an odd prime $q$ and a generic block $c = (c_{1}, c_{2}, c_{3})$ in the sense of O12,
    that is $c_{i} \not\equiv 0$ and $\sum_{i} c_{i} \not\equiv 0 \pmod{q}$.
    The block is a triple, whereas the Heisenberg and Weil actions on $\mathbb{C}^{q}$ depend on
    a single central character; the relevant scalar is
    \[
      c_{\Sigma} = c_{1} + c_{2} + c_{3} \bmod q,
    \]
    which the genericity condition makes nonzero, and which is the character under which a
    $k = 3$ fingerprint vector transforms.
    All representations below are taken at $c_{\Sigma}$ and written $\rho_{c_{\Sigma}}$ and
    $W_{c_{\Sigma}}$.
    ...
    $W_{<n}$.
    The symmetrised Weyl operators are
    \[
      \big(W_{c_{\Sigma}}(a,b) f\big)(x)
      = e_{q}\!\big(c_{\Sigma} b (x - a/2)\big)\, f(x-a),
      \qquad e_{q}(t) = \exp(2\pi \mathrm{i} t / q),
    \]
    with $a/2$ read in $\mathbb{F}_{q}$, and they form an orthogonal basis of
    $\mathrm{End}(\mathbb{C}^{q})$.
    The Weil representation $\rho_{c_{\Sigma}}$ of $\SL(2,\mathbb{Z}/q\mathbb{Z})$ acts on the
    same $\mathbb{C}^{q}$, exists as a genuine linear representation because $q$ is an odd
    prime~\cite{Weil1964,Gerardin1977}, and normalises the Heisenberg action.
    In this symmetrised normalisation the intertwining relation holds on the nose,
    \begin{equation}
      \label{eq:intertwining}
      \rho_{c_{\Sigma}}(g)\, W_{c_{\Sigma}}(v)\, \rho_{c_{\Sigma}}(g)^{-1}
      = W_{c_{\Sigma}}(gv),
      \qquad g \in \SL(2,\mathbb{Z}/q\mathbb{Z}),\ v \in \mathbb{F}_{q}^{2},
    \end{equation}
    with no residual phase; this is the defining property of the Weil representation in
    Gérardin's construction~\cite[\S1--2]{Gerardin1977} and is what licenses the argument
    below.
    A projective lift would satisfy~\eqref{eq:intertwining} equally, since conjugation cancels
    scalars, so the argument does not depend on which lift is used.
    ...
    \begin{theorem}[Exact stabiliser of an O12 filtration level]
      \label{thm:o12-stabiliser}
      For every proper nonzero level, that is whenever $0 < |F_{n}| < q$,
      \[
        \mathrm{Stab}(W_{<n})
        = \{ g \in \SL(2,\mathbb{Z}/q\mathbb{Z}) : \rho_{c_{\Sigma}}(g) W_{<n} = W_{<n} \}
        = U \rtimes M_{n}
        \subseteq B(L).
      \]
    \end{theorem}
    ...
    \begin{proof}
      Expand $P_{n} = q^{-1} \sum_{v} p_{n}(v) W_{c_{\Sigma}}(v)$ in the Weyl operator basis of
      $\mathrm{End}(\mathbb{C}^{q})$, with
      $p_{n}(v) = \mathrm{tr}(P_{n} W_{c_{\Sigma}}(v)^{\dagger})$.
      Since $\rho_{c_{\Sigma}}(g)$ is unitary, $\rho_{c_{\Sigma}}(g) W_{<n} = W_{<n}$ is
      equivalent to
      $\rho_{c_{\Sigma}}(g) P_{n} \rho_{c_{\Sigma}}(g)^{\dagger} = P_{n}$, and
      by~\eqref{eq:intertwining} this holds if and only if $p_{n} \circ g^{-1} = p_{n}$ on
      $\mathbb{F}_{q}^{2}$.
      By Lemma~\ref{lem:pure-fourier}, $P_{n}$ is diagonal in the Fourier basis, so $p_{n}$
      vanishes off $L$.
      Containment of the support in $L$ does not by itself force a stabilising $g$ to preserve
      $L$, so the argument is completed as follows.
      In the Fourier basis $P_{n}$ is the diagonal indicator of $F_{n}$, and for $v = (a,0)$ the
      coefficient $p_{n}(a,0)$ is, up to a fixed phase, the discrete Fourier transform of that
      indicator evaluated at $a$.
      If $p_{n}(a,0)$ vanished for every $a \neq 0$, the indicator of $F_{n}$ would have a flat
      transform and would therefore be constant, forcing $|F_{n}| \in \{0, q\}$, which the
      hypothesis $0 < |F_{n}| < q$ excludes.
      Hence some $v_{0} = (a_{0},0)$ with $a_{0} \neq 0$ has $p_{n}(v_{0}) \neq 0$, and since
      $q$ is prime that single nonzero vector spans $L$.
      A stabilising $g$ preserves the whole coefficient function, hence maps the nonempty set
      $\{v \neq 0 : p_{n}(v) \neq 0\} \subseteq L \setminus \{0\}$ to itself, hence preserves
      the line spanned by any of its elements, which is $L$.
      Therefore $\mathrm{Stab}(W_{<n}) \subseteq B(L)$.
      ...
      In the deposited normalisation the torus element $\mathrm{diag}(s, s^{-1})$ acts by
      $f \mapsto f(s^{-1} \cdot)$, hence sends $e_{f}$ to $e_{s^{-1} f}$.
      It therefore preserves $W_{<n}$ if and only if $s^{-1} F_{n} = F_{n}$, which, $M_{n}$
      being a group, is the same condition as $s F_{n} = F_{n}$, that is $s \in M_{n}$.
    \end{proof}

    \begin{corollary}[No exceptional binary polyhedral group is selected]
    ...
    \begin{remark}[The exclusion does not extend to dicyclic groups]
      \label{rem:dicyclic-not-excluded}
      Corollary~\ref{cor:no-binary-polyhedral} concerns the \emph{exceptional} binary polyhedral
      groups only, and no inference to ``no two-dimensional carrier'' is available from it.
      The finite subgroups of $\SU(2)$ also include the dicyclic (binary dihedral) family
      $\mathrm{Dic}_{m}$, listed among the admissible candidates in
      Remark~\ref{rem:exclusion-scope}, and those groups \emph{are} metacyclic: $\mathrm{Dic}_{m}$
      has a cyclic normal subgroup of order $2m$ with quotient of order two.
      Metacyclicity is only the removal of the obstruction, not an embedding: being metacyclic
      does not by itself place a group inside $B(L)$, and no such general implication is used
      here.
      What establishes the point is explicit realisation.
      Proposition~\ref{prop:dicyclic-witnesses} exhibits levels at which
      $\mathrm{Stab}(W_{<n})$ \emph{is} dicyclic, produced by the exact formula of
      Theorem~\ref{thm:o12-stabiliser} itself, and those groups carry faithful irreducible
      complex representations of dimension two.
      Proposition~\ref{prop:dicyclic-witnesses} shows that this is not a hypothetical gap:
      the exact formula of Theorem~\ref{thm:o12-stabiliser} produces dicyclic stabilisers at
      genuine levels of the deposited construction.
    \end{remark}

    \begin{proposition}[Two exact dicyclic witnesses with multiplicity-free spinorial content]
      \label{prop:dicyclic-witnesses}
      Let $(q, c, n)$ be either $(53, (47,21,32), 1)$ or $(101, (41,95,6), 1)$; both blocks are
      generic in the sense of O12.
      At these levels $\dim W_{<1} = 9$, the multiplier group is
      $M_{1} = \{1, s, s^{-1}, -1\}$ with $s^{2} = -1$, and
      \[
        \mathrm{Stab}(W_{<1}) = U \rtimes M_{1} \cong \mathrm{Dic}_{q},
        \qquad |\mathrm{Stab}(W_{<1})| = 4q,
      \]
      with the standard presentation realised by $a = (-\mathbb 1)u$ of order $2q$ and
      $x = \mathrm{diag}(s,s^{-1})$ of order four, satisfying $x^{2} = -\mathbb 1 = a^{q}$ and
      $x^{-1} a x = a^{-1}$.
      Restricted to this stabiliser, $W_{<1}$ decomposes multiplicity-freely as
      \[
        W_{<1} \;\cong\; \mathbf{1}' \oplus V_{1} \oplus V_{2} \oplus V_{3} \oplus V_{4},
        \qquad \dim_{\mathbb{C}} V_{i} = 2,
      \]
      with $\mathbf{1}'$ a one-dimensional character, the $V_{i}$ pairwise inequivalent
      two-dimensional irreducible representations of $\mathrm{Dic}_{q}$, every multiplicity
      equal to one, and $1 + 4 \times 2 = 9$.

      Exactly two of the four are admissible carriers.
      Write $V_{j}$, for $1 \le j \le q-1$, for the two-dimensional irreducible determined by
      $\rho_{j}(a) = \mathrm{diag}(\omega^{j}, \omega^{-j})$ with $\omega$ a primitive
      $2q$-th root of unity and
      $\rho_{j}(x) = \left(\begin{smallmatrix} 0 & 1 \\ (-1)^{j} & 0 \end{smallmatrix}\right)$,
      so that $\chi_{j}(a^{m}) = 2\cos(\pi j m / q)$.
      Two conditions must be checked, and they coincide.
      \emph{Faithfulness.} $\rho_{j}$ is faithful exactly when $\rho_{j}(a)$ has order $2q$,
      that is when $\omega^{j}$ is again a primitive $2q$-th root of unity, that is when
      $\gcd(j, 2q) = 1$.
      Since $q$ is prime and $1 \le j \le q-1$, the factor $q$ never divides $j$, so
      $\gcd(j, 2q) = 1$ reduces to $\gcd(j,2) = 1$, i.e.\ to $j$ odd.
      Equivalently, the central element $a^{q} = -\mathbb 1$ acts by $(-1)^{j}$, and acts
      trivially exactly when $j$ is even.
      \emph{Determinant one.} Being $\SU(2)$-valued in the sense required by
      Theorem~\ref{thm:adjoint-dimension} additionally requires $\det \rho_{j} \equiv 1$.
      Here $\det \rho_{j}(a) = \omega^{j} \omega^{-j} = 1$ always, while
      $\det \rho_{j}(x) = -(-1)^{j} = (-1)^{j+1}$, which equals $1$ exactly when $j$ is odd.
      Both conditions therefore select the same indices, and $V_{j}$ is an admissible carrier if
      and only if $j$ is odd.
      The constituents are $j \in \{6, 11, 42, 47\}$ at $q = 53$ and
      $j \in \{12, 41, 60, 89\}$ at $q = 101$, so the admissible ones are $j \in \{11, 47\}$
      and $j \in \{41, 89\}$ respectively.
      For even $j$ the central element acts trivially, the representation factors through the
      dihedral quotient, and it is not a spinor carrier at all.
    \end{proposition}

    \begin{proof}[Verification]
      The group-theoretic statements are exact modular arithmetic on the sets $F_{1}$ and
      $M_{1}$ and are reproduced by the committed code.
      The decomposition is obtained from the character
      $\chi(g) = \mathrm{tr}\big(\rho_{c_{\Sigma}}(g)|_{W_{<1}}\big)$ against the character
      table of $\mathrm{Dic}_{q}$, giving $\langle \chi, \chi \rangle = 5$ with five distinct
      constituents, and is confirmed by constructing the isotypic projectors: each is idempotent
      to $2 \times 10^{-14}$, has rank $\dim_{\mathbb{C}} V_{i} \cdot m_{i}$ as predicted, and
      they sum to the identity on $W_{<1}$ to the same order.
      Invariance of $W_{<1}$ under the whole stabiliser holds to $6 \times 10^{-14}$.
    \end{proof}

    \begin{remark}[Status of Proposition~\ref{prop:dicyclic-witnesses} and of the lift]
      \label{rem:witness-status}
      Proposition~\ref{prop:dicyclic-witnesses} is a computational result on two explicit
      witnesses, stated with its blocks and levels inscribed in the statement.
      It is not a universal claim: it is not asserted here that every dicyclic level decomposes
      this way, nor that dicyclic levels occur for every prime.
      The group-theoretic part is exact integer arithmetic; the decomposition is computed in
      floating point with the thresholds recorded in the repository.

      One further qualification concerns the representation used.
      A genuine linear Weil representation exists for $q$ an odd
      prime~\cite{Weil1964,Gerardin1977}; the implementation builds it from Bruhat words in
      fixed generators, and that this particular lift is the genuine one rather than a
      projective twist is verified numerically --- the homomorphism defect vanishes to
      $10^{-14}$ over all ordered pairs of the stabiliser --- and is not proved here.
      The scope of that verification should be stated precisely, since a tempting robustness
      argument does not in fact remove the gap.
      \emph{If} two maps into the unitary group are both homomorphisms lifting the same
      projective representation, then they differ by a linear character, and tensoring by a
      linear character permutes the irreducible constituents while preserving their dimensions
      and the multiplicity-free property.
      That argument presupposes what is being checked: it compares two homomorphisms, and says
      nothing if the constructed map fails to be one.
      The homomorphism property is therefore established here numerically --- the defect
      vanishes to $10^{-14}$ over all $|G|^{2}$ ordered pairs of the stabiliser, which is an
      exhaustive check on the group that matters, not a sample --- and the character
      computation is conditional on it.
      What the twisting argument does give, once the map is a homomorphism, is that the
      \emph{labels} $j$ of the components are convention-dependent while the shape of the
      decomposition is not.
    \end{remark}

    \begin{corollary}[The obstruction is selection, not existence]
      \label{cor:selection-not-existence}
      At the levels of Proposition~\ref{prop:dicyclic-witnesses} the O12 filtration selects a
      subgroup carrying genuine two-dimensional spinorial content, so Borel confinement does not
      by itself obstruct a spinor carrier.
      It supplies exactly two admissible carriers, the faithful constituents $V_{j}$ with $j$
      odd.
      Each of them is a well-defined subspace of $W_{<1}$: the decomposition is
      multiplicity-free, so every isotypic component is mathematically canonical, and no
      arbitrary choice of basis or complement is involved in naming them.
      What has \emph{not} been \emph{derived} is a rule selecting one of the two, that is a
      justified selector
      \[
        \{V_{j} : j \text{ odd}\} \longrightarrow V_{\rho}.
      \]
      A candidate is available and survives every audit performed
      (Remark~\ref{rem:o12-scope}): take the unique odd member of
      $\{c_{\Sigma},\, q - c_{\Sigma}\}$.
      Bridge~1 is therefore displaced rather than closed, and the residual gap is one proof step
      inside a specific candidate rule, not an unstructured choice among carriers.
      No claim is made that the ambiguity is irreducible.
    \end{corollary}

    \begin{remark}[What is universal, what is measured, and what is not established]
      \label{rem:o12-status}
      Statements of four different strengths must be kept apart, and the distinction between a
      single \emph{level} and the \emph{full filtration} matters throughout.
      Theorem~\ref{thm:o12-stabiliser} and Corollary~\ref{cor:no-binary-polyhedral} are
      universal statements about one proper nonzero level: they hold for every odd prime, every
      generic block and every such level.
      Proposition~\ref{prop:dicyclic-witnesses} is a computational result on two named levels.
      Neither of the stronger claims ``every full O12 filtration has a dicyclic stabiliser'' nor
      ``no full O12 filtration can have one'' is established here; the full-filtration
      stabiliser is $\bigcap_{n} \mathrm{Stab}(W_{<n})$ and is a different object, always
      contained in each level stabiliser.
      ...
      \emph{not established}: it held in every one of the $12\,008$ draws performed --- draws
      with replacement, not necessarily distinct triples --- and no proof is offered.
      The universal statements do not depend on any of these measurements; the excess
      multipliers are precisely what produces the dicyclic levels of
      Proposition~\ref{prop:dicyclic-witnesses}, so they are read here as a positive lead rather
      than as sampling noise.
    \end{remark}

    \begin{remark}[Scope of the obstruction]
      \label{rem:o12-scope}
      Corollary~\ref{cor:no-binary-polyhedral} rules out the exceptional groups only, and it is
      not a no-go on the value three.
      Bridge~1 remains open, now in a sharper form: by
      Corollary~\ref{cor:selection-not-existence} the missing object is a selector among
      inequivalent two-dimensional components, not a two-dimensional module.
      Two structural consequences are worth recording for future attempts.
      First, enriching the object cannot enlarge the stabiliser: if $g$ fixes a set of vectors,
      a labelled shell family, or a configuration carrying the BFS metric, then it fixes their
      linear span, so the stabiliser of any such configuration is contained in
      $\mathrm{Stab}(W_{<n})$.
      Second, the components are individually well defined, since the decomposition is
      multiplicity-free, and exactly two of them are admissible carriers.
      Whether a derived rule picks one of the two is the remaining question, and the evidence
      currently favours an affirmative answer without settling it.

      The oriented rule ``take $j = c_{\Sigma}$'' fails.
      At $q = 53$ with the generic block $c = (10, 35, 18)$ the level is dicyclic with
      constituents $j \in \{10, 17, 36, 43\}$ and admissible carriers $j \in \{17, 43\}$,
      while $c_{\Sigma} = 10$ is a constituent but is even, hence not faithful.
      Over the levels audited it succeeds in $6$ of $14$ cases.

      The sign-completed rule does not fail.
      Take the pair $\{c_{\Sigma},\, q - c_{\Sigma}\}$ and select its unique odd member.
      In the counterexample above this selects $j = 43$, an admissible carrier.
      Two ingredients of this rule are immediate.
      Since $q$ is odd, exactly one of $c_{\Sigma}$ and $q - c_{\Sigma}$ is odd, so the rule
      always returns a single index; and both $\pm c_{\Sigma}$ always lie in $F_{1}$, because
      the fingerprint frequency is $\sum_{i} c_{i} b_{i}$ and the generator word taking
      $b_{1} = b_{2} = b_{3} = \pm 1$ is available in the deposited generating set.
      The remaining step, that the pair $\{c_{\Sigma}, q - c_{\Sigma}\}$ appears among the
      labels of the two-dimensional constituents, is observed and not proved.
      It held, together with the selection of an admissible carrier, in every case audited.
      Two counts are reported, because the selector is a function of the pair
      $(W_{<1}, \rho_{c_{\Sigma}})$ and not of the subspace alone: distinct blocks may realise
      the same subspace while carrying different ambient central characters, hence different
      instances of the rule.
      The audit comprises $17$ typed instances --- three deposited decompositions and fourteen
      sweep rows --- realised on $13$ distinct underlying subspaces $W_{<1}$, and the rule
      succeeded on all $17$.
      At $q = 101$ the sweep stopped on its draw budget of $20\,000$ random blocks rather than
      on an exhaustion of the admissible blocks, returning six instances instead of the eight
      requested; the run reports this.

      Consequently no claim of an irreducible twofold ambiguity is made.
      What is established is that two admissible carriers are present and that a structurally
      motivated candidate selector --- structural because $c_{\Sigma}$ is the ambient central
      character of the construction, not a free parameter --- is consistent with every level
      examined.

      The scope of that statement must be read strictly, and it does not reduce Bridge~1 as a
      whole.
      The rule operates \emph{inside} a level that has already been shown to be dicyclic; it
      presupposes a choice of prime $q$, of generic block $c$, and of level $n$.
      No result of this paper selects those data within the generic filtration: dicyclic levels
      are a sparse phenomenon among generic blocks, and nothing here says which level of which
      block the projection is supposed to single out.
      Bridge~1 therefore retains at least two open steps --- the selection of an admissible
      level, and the proof of the observed step inside it --- and only the second is reduced to
      a well-posed proof obligation.

      A test that does \emph{not} settle the question should also be recorded, since it is the
      first that comes to mind.
      The normaliser of the stabiliser does not act on the components, because its elements
      outside the stabiliser do not preserve $W_{<1}$; asking whether it permutes them
      transitively is not well posed.
    \end{remark}
  ...
  \subsection{What O23 establishes and what it does not}
    ...
    It establishes further, and unconditionally, the exact stabiliser of every proper nonzero
    level of the O12 shell-span filtration in the Weil image
    (Theorem~\ref{thm:o12-stabiliser}) and the resulting exclusion of the exceptional binary
    polyhedral groups (Corollary~\ref{cor:no-binary-polyhedral}), and it exhibits two explicit
    levels whose stabiliser is dicyclic and whose restricted Weil module is multiplicity-free
    with four two-dimensional constituents (Proposition~\ref{prop:dicyclic-witnesses}).
    ...
    What Section~\ref{ssec:o12-obstruction} contributes to the first is a change in its type,
    and a decomposition into two steps rather than one.
    Before, Bridge~1 asked for a two-dimensional carrier and no source was known to produce one.
    Now a source is known to produce two admissible ones, canonically defined and inequivalent,
    at explicit levels.
    The bridge accordingly splits.
    Its first step is the selection of an admissible level: which prime, which generic block and
    which depth the projection is supposed to single out within the generic filtration, dicyclic
    levels being sparse among generic blocks.
    Nothing in this paper addresses that step, and it is not reduced by anything proved here.
    Its second step, internal to a level already known to be dicyclic, is a justified selector
    between the two admissible carriers; that step is testable, and a candidate can be checked
    against the components directly, as is done in Remark~\ref{rem:o12-scope}, where the
    oriented rule $j = c_{\Sigma}$ is refuted and its sign-completed form is not.
    Only the second step is reduced to a well-posed proof obligation; the first remains
    untouched.
  ...
  On the first bridge, one candidate source is now analysed exactly.
  Every proper nonzero level of the O12 shell-span filtration has stabiliser $U \rtimes M_{n}$
  inside a Borel subgroup of $\SL(2,\mathbb{Z}/q\mathbb{Z})$
  (Theorem~\ref{thm:o12-stabiliser}), which excludes the exceptional binary polyhedral groups
  $2T$, $2O$, $2I$ even at the primes where they exist in the ambient group
  (Corollary~\ref{cor:no-binary-polyhedral}).
  It does not exclude a spinor carrier.
  Dicyclic groups are metacyclic, and at explicit generic levels at $q = 53$ and $q = 101$ the
  stabiliser is $\mathrm{Dic}_{q}$, whose restricted Weil module is one character plus four
  pairwise inequivalent two-dimensional irreducible representations, each of multiplicity one,
  exactly two of them faithful and hence admissible carriers
  (Proposition~\ref{prop:dicyclic-witnesses}).
  Each component is canonical, the decomposition being multiplicity-free.
  What has not been \emph{proved} is the rule selecting one of the two admissible carriers, for
  which a structurally motivated candidate --- the unique odd member of
  $\{c_{\Sigma}, q - c_{\Sigma}\}$ --- succeeds on all thirteen distinct levels examined
  (Corollary~\ref{cor:selection-not-existence}, Remark~\ref{rem:o12-scope}).
  That candidate presupposes a level already known to be dicyclic, and no result here selects
  the prime, block and level within the generic filtration, so Bridge~1 retains two open steps
  rather than one.
  What the analysis removes is the reading in which the source supplies no two-dimensional
  module at all.

  \paragraph{Interpretive outlook (interpretation, not a result).}
    Read structurally, the admissible filtration turns out to be neither silent nor decisive
    about spin.
    It does produce, at some levels, a group of exactly the kind that can carry a spinor, and
    genuine two-dimensional content inside the admissible level itself; what it does not
    yet prove is the rule that picks one of the two available spinor carriers, although a
    candidate reading it off the ambient central character works in every case examined.
    If the three neutral directions of Theorem~\ref{thm:adjoint-dimension} are eventually to
    explain why observable structure comes in threes, the missing ingredient is now visible and
    smaller and better localised than before: selecting an admissible level, and one proof step
    inside an identified candidate rule, rather than a construction of the carrier from
    nothing.
    This reading is offered as orientation, not as an established consequence: the paper locates
    the remaining gap, and does not close it.
