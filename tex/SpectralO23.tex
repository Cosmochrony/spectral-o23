...
\begin{document}
  ...
  \begin{abstract}
    ...
    representations.
    That source therefore supplies genuine spinorial content and no distinguished carrier, so
    the residual obstruction is the absence of a canonical selector among four components rather
    than the non-existence of a two-dimensional module.
  ...
  \subsection{A proved obstruction on Bridge 1: the O12 filtration cannot supply the carrier}
    ...
    \begin{remark}[Status of Proposition~\ref{prop:dicyclic-witnesses} and of the lift]
      ...
      The conclusion is nevertheless insensitive to that gap.
      Two lifts of a projective representation differ by a linear character of the group, and
      tensoring by a linear character permutes the irreducible constituents while preserving
      their dimensions and the multiplicity-free property.
      The shape $1 + 4 \times 2$ and the multiplicity-freeness are therefore independent of the
      choice of lift; only the labels of the four two-dimensional components depend on it.
    \end{remark}
    ...
    \begin{corollary}[The obstruction is selection, not existence]
      ...
      It does not, however, select \emph{a} carrier: the spinorial part of $W_{<1}$ is
      multiplicity-free with four pairwise inequivalent two-dimensional components, and nothing
      in the construction distinguishes one of them.
      Bridge~1 is therefore displaced rather than closed: what is missing is not the existence
      of a two-dimensional module but a canonical selector
      \[
        \{V_{1}, V_{2}, V_{3}, V_{4}\} \longrightarrow V_{\rho}.
      \]
    \end{corollary}
    ...
    \begin{remark}[Scope of the obstruction]
      ...
      $\mathrm{Stab}(W_{<n})$.
      Second, the four two-dimensional components are individually well defined --- the
      decomposition is multiplicity-free, so each isotypic component is a canonical subspace of
      $W_{<1}$ --- but nothing exhibited here distinguishes one of them from the others.
      Whether a canonical selector exists is open, and is not settled either way by this paper.
      The natural next test is whether the normaliser of the stabiliser permutes the four
      components transitively, since a transitive action would show that no selector can be
      definable from the level and its stabiliser alone, while a non-transitive one would
      identify invariantly distinguishable candidates.
    \end{remark}
  ...
  \subsection{What O23 establishes and what it does not}
    ...
    Now a source is known to produce four, canonically defined and pairwise inequivalent, at
    explicit levels; the bridge asks for a selector among them.
    That is a strictly smaller gap, and a testable one: a candidate selector can be checked
    against the four components directly.
  ...
  The filtration therefore supplies genuine spinorial content and no distinguished carrier, so
  the residual obstruction on this bridge is a selection problem rather than an existence
  problem (Corollary~\ref{cor:selection-not-existence}).
  ...
  \paragraph{Interpretive outlook (interpretation, not a result).}
    ...
    produce is a reason to prefer one of the four available spinor directions over the others.
    ...
    much smaller than before: a principle that breaks a fourfold degeneracy, rather than a
