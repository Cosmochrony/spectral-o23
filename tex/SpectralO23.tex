...
\begin{document}
  ...
  \begin{abstract}
    ...
    decomposition is multiplicity-free; what it does not supply is a derived reason to prefer one
    admissible carrier over the other.
    The residual obstruction on this bridge is therefore a twofold selection problem, not the
    non-existence of a two-dimensional module.
  ...
  \subsection{What the O12 filtration supplies on Bridge 1, and what it does not}
    ...
    \begin{corollary}[The obstruction is selection, not existence]
      ...
      What has \emph{not} been derived is a structural or physical reason to prefer one of the
      two, that is a justified selector
      \[
        \{V_{j} : j \text{ odd}\} \longrightarrow V_{\rho}.
      \]
      Bridge~1 is therefore displaced rather than closed, and the residual ambiguity is twofold
      rather than fourfold.
      No claim is made here that such a selector cannot exist; only that none is derived.
    \end{corollary}
    ...
    \begin{remark}[Scope of the obstruction]
      ...
      multiplicity-free, and exactly two of them are admissible carriers; what is absent is a
      derived reason to prefer one.
      One internal candidate suggests itself and is recorded here together with its refutation,
      to prevent it being rediscovered and mistaken for a rule.
      In both witnesses the ambient central character $c_{\Sigma}$ is itself the index of an
      admissible constituent: $c_{\Sigma} = 47$ at $q = 53$ and $c_{\Sigma} = 41$ at
      $q = 101$, both odd and both among the constituents.
      The rule ``take $j = c_{\Sigma}$'' nevertheless fails.
      At $q = 53$ with the generic block $c = (10, 35, 18)$ the level is again dicyclic with
      constituents $j \in \{10, 17, 36, 43\}$ and admissible carriers $j \in \{17, 43\}$,
      while $c_{\Sigma} = 10$ is a constituent but is even, hence not faithful and not a
      carrier at all.
      Across the four distinct dicyclic levels examined, $c_{\Sigma}$ was always among the four
      constituents but was an admissible carrier in three of them.
      The regularity $c_{\Sigma} \in \{j\}$ is therefore real and unexplained, and is worth
      investigating, but it does not by itself furnish a selector.
  ...
  \subsection{What O23 establishes and what it does not}
    ...
    against the components directly, as is done in Remark~\ref{rem:o12-scope} for the rule
    $j = c_{\Sigma}$, which the check refutes.
  ...
  Each component is canonical, the decomposition being multiplicity-free; what has not been
  derived is a reason to prefer one admissible carrier over the other, so the residual
  obstruction on this bridge is a twofold selection problem rather than an existence problem
  (Corollary~\ref{cor:selection-not-existence}).
  ...
  \paragraph{Interpretive outlook (interpretation, not a result).}
    ...
    produce is a reason to prefer one of the two available spinor carriers over the other.
    ...
    much smaller than before: a principle that breaks a twofold degeneracy, rather than a
