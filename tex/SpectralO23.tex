...
\begin{document}
  ...
  \begin{abstract}
    ...
    irreducible two-dimensional unitary representation $V_{\rho} \cong \mathbb{C}^{2}$, then
    its traceless neutral sector is the real Lie algebra $\su(2) \cong \ImH$, of real
    ...
    O18~\cite{Beau2026a22} is an equivariant symmetry and does not force the fibre
    structure, so the spinor carrier is a supplied input.
  ...
  \subsection{Scope and what this paper does not claim}
    ...
    directions: the traceless neutral sector of an irreducible two-dimensional unitary
    carrier is $\su(2) \cong \ImH$, of real dimension three.
  ...
  \subsection{Non-abelian support of the carrier}
    ...
    \begin{remark}[Candidate groups]
      ...
      Given the irreducible two-dimensional unitary carrier, the candidate groups are the
      finite non-abelian groups admitting a faithful irreducible two-dimensional
      representation.
  ...
  \subsection{The adjoint-dimension theorem}
    ...
    \begin{theorem}[Three neutral directions from a spinor carrier]
      ...
      Let $V_{\rho} \cong \mathbb{C}^{2}$ be an irreducible two-dimensional unitary
      ...
      lies on the unit sphere of this three-dimensional space.
    \end{theorem}
    ...
    \begin{proof}
      ...
      Lie algebra isomorphism $\ImH \to \su(2)$ (with the commutator bracket scaled by the
      standard factor two), realising the classical identification $\su(2) \cong
      \mathfrak{sp}(1) \cong \ImH$.
      By~\eqref{eq:pauli-axis}, each neutral generator image is
      $\mathrm{i}(\vec{u}_{s} \cdot \vec{\sigma})$ with $|\vec{u}_{s}| = 1$, an element of
      norm one in this space.
    \end{proof}
  ...
  \subsection{Bridge 1: carrier selection}
    ...
    structure.
    O18~\cite{Beau2026a22} establishes that Born--Infeld parity is an equivariant symmetry
    of the effective responses (O18 Proposition~2.6) and that evenness of the action does
    not force parity-related configurations into a common fibre (O18 Proposition~2.4);
  ...
  \subsection{The honest chain}
    ...
    \begin{align}
      \label{eq:chain}
      &\text{BI parity (equivariant, proved)}
  ...
  \subsection{What O23 establishes and what it does not}
    ...
    O23 establishes, unconditionally for a supplied irreducible two-dimensional unitary
    carrier, that the neutral traceless sector is $\su(2) \cong \ImH$ with exactly three
  ...
  irreducible two-dimensional unitary carrier $V_{\rho}$, the traceless neutral sector is
