...
\begin{document}
  ...
  \begin{abstract}
    O21 defined the observable shell $n_{3}$ via the threshold condition
    ...
    This is a representation-theoretic theorem about the supplied carrier, and it explains why a
    spinorial neutral sector carries three independent directions.
    Two bridges separate it from a derivation of $\Sigma_{c}(n_{3}) = 3$, and both are open.

    The first is carrier selection, and the paper determines exactly what one candidate source
    supplies.
    The fingerprint vectors of the deposited O12 shell-span filtration are pure Fourier modes,
    so every proper level is a coordinate subspace whose Weyl support is a single line, and the
    exact stabiliser of that level in the Weil image of $\SL(2,\mathbb{Z}/q\mathbb{Z})$ is the
    metacyclic group $U \rtimes M_{n}$ inside the corresponding Borel subgroup.
    This excludes the exceptional binary polyhedral groups $2T$, $2O$ and $2I$ at every proper
    level, including at the primes where they exist in the ambient group.
    It does not exclude a spinor carrier.
    Dicyclic groups are metacyclic, and at explicit generic levels at $q = 53$ and $q = 101$ the
    stabiliser is $\mathrm{Dic}_{q}$, whose restricted Weil module decomposes multiplicity-freely
    as one character together with four pairwise inequivalent two-dimensional irreducible
    representations, of which exactly two are faithful and therefore admissible carriers.
    That source supplies genuine spinorial content, and each component is canonical because the
    decomposition is multiplicity-free; what it does not yet supply is a \emph{proved} rule
    selecting one admissible carrier.
    A structurally motivated candidate is available and survives every audit performed: take the
    unique odd member of the pair $\{c_{\Sigma},\, q - c_{\Sigma}\}$ formed from the ambient
    central character.
    It selects an admissible carrier in all thirteen distinct dicyclic levels examined, and one
    step of its justification remains unproved.
    That candidate operates inside a level already known to be dicyclic, so the bridge as a whole
    is not reduced to it: selecting which prime, block and level the projection should single out
    within the generic filtration remains a separate open step.
    What the analysis removes is the non-existence reading: the source does supply
    two-dimensional modules, and the residual difficulty is selection at two distinct levels of
    the construction.

    The second bridge is observable identification: no theorem identifies the cumulative
    Gram--Schmidt span $\Sigma_{c}$ of O21 with the dimension of the neutral traceless module,
    and since $n_{3}$ is defined as the shell where $\Sigma_{c}$ reaches three, the threshold
    remains a supplied selection rule.
    A structural caution accompanies the positive result: in the faithful two-dimensional
    representation of $Q_{8}$ the set $\{\pm\mathbf{i}, \pm\mathbf{j}\}$ generates the group
    with only two generator axes, the third direction arising only under commutator closure.
    The ADE observation of three non-trivial eigenvalue classes is retained as a consistency
    check compatible with, but not probative of, the threefold structure.
  \end{abstract}
  ...
  \subsection{What the O12 filtration supplies on Bridge 1, and what it does not}
    ...
    \begin{proposition}[Two exact dicyclic witnesses with multiplicity-free spinorial content]
      ...
      Writing $V_{j}$ for the two-dimensional irreducible with
      $\chi_{j}(a^{m}) = 2\cos(\pi j m / q)$, the central element $a^{q} = -\mathbb 1$ acts by
      $(-1)^{j}$, so $V_{j}$ is faithful, and $\SU(2)$-valued in the sense required by
      Theorem~\ref{thm:adjoint-dimension}, if and only if $j$ is odd.
    ...
    \begin{remark}[Scope of the obstruction]
      ...
      It held, together with the selection of an admissible carrier, in every level audited.
      The count is stated in distinct exact subspaces rather than in runs: the three levels with
      deposited decompositions and the deposited selector sweep together comprise $13$ distinct
      subspaces $W_{<1}$ ($14$ sweep rows realise $11$ distinct subspaces, of which $10$ are new
      relative to the three witnesses), and the rule succeeded on all $13$.
      The sweep also exhausted the available blocks at $q = 101$ before reaching its quota,
  ...
  \subsection{What O23 establishes and what it does not}
    ...
    at explicit levels; the bridge asks for a justified selector between them.
    That is a strictly smaller gap, and a testable one: a candidate selector can be checked
